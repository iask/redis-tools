...
def client(host='localhost', port=6379, password=None, db=0,
                 unix_socket_path=None, encoding='utf-8', cluster=None):
...
def dumps(host='localhost', port=6379, password=None, db=0, pretty=False,
    ...
    r = client(host=host, port=port, password=password, db=db,
               unix_socket_path=unix_socket_path, encoding=encoding, cluster=cluster)
    kwargs = {}
    if not pretty:
        kwargs['separators'] = (',', ':')
    else:
    ...
    table = {}
    if cluster:
        nodes = get_cluster_nodes(host=host, port=port, password=password, db=db,
                                  unix_socket_path=unix_socket_path, encoding=encoding)
    else:
        nodes = [[host, port]]
    for node_host, node_port in nodes:
        rs = client(host=node_host, port=node_port, password=password, db=db,
                    unix_socket_path=unix_socket_path, encoding=encoding)
        for key, type, value in _reader(r, pretty, encoding, rs):
...
def dump(fp, host='localhost', port=6379, password=None, db=0, pretty=False,
    ...
    if pretty:
        ...
        fp.write(dumps(host=host, port=port, password=password, db=db,
            pretty=pretty, encoding=encoding, cluster=cluster))
        return
    kwargs = {}
    if not pretty:
        kwargs['separators'] = (',', ':')
...
def get_cluster_nodes(host='localhost', port=6379, password=None, db=0, pretty=False,
         unix_socket_path=None, encoding='utf-8'):
    rs = client(host=host, port=port, password=password, db=db, unix_socket_path=unix_socket_path, encoding=encoding)
    cluster_nodes = rs.execute_command("cluster nodes"); 
...
def loads(s, host='localhost', port=6379, password=None, db=0, empty=False,
          unix_socket_path=None, encoding='utf-8', cluster=None,):
    r = client(host=host, port=port, password=password, db=db, unix_socket_path=unix_socket_path, encoding=encoding, cluster=cluster)
    if empty:
        _empty(r)
    table = json.loads(s)
    counter = 0
...
def load_lump(fp, host='localhost', port=6379, password=None, db=0,
    empty=False, unix_socket_path=None, encoding='utf-8', cluster=None
):
    s = fp.read()
    if py3:
        ...
        if isinstance(s, bytes):
            s = s.decode(encoding)
    loads(s, host, port, password, db, empty, unix_socket_path, encoding, cluster)
...
def load_streaming(fp, host='localhost', port=6379, password=None, db=0,
    ...
    streaming_backend=None, cluster=None,
):
    r = client(host=host, port=port, password=password, db=db,
               unix_socket_path=unix_socket_path, encoding=encoding, cluster=cluster)
...
def load(fp, host='localhost', port=6379, password=None, db=0,
    ...
    if have_streaming_load:
        load_streaming(fp, host=host, port=port, password=password, db=db,
            ...
            streaming_backend=streaming_backend, cluster=cluster)
    else:
        load_lump(fp, host=host, port=port, password=password, db=db,
            empty=empty, unix_socket_path=unix_socket_path, encoding=encoding, cluster=cluster)
...
if __name__ == '__main__':
    ...
    parser.add_option('-c', '--cluster', help='server cluster mode enable', action='store_true')
    if help == DUMP:
